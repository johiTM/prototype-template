...
- **Format:** Markdown (`.md`)
- **Location:** `.cursor/documents/tasks/`
...
9.  **Save Task List:** Save the generated document in the `.cursor/documents/tasks/` directory with the filename `tasks-[prd-file-name].md`, where `[prd-file-name]` matches the base name of the input PRD file (e.g., if the input was `0001-prd-user-profile-editing.md`, the output is `tasks-0001-prd-user-profile-editing.md`).
...
- `index.html` - Brief description (e.g., Entry page or additional screen).
- `css/feature-name.css` - Brief description (e.g., Styles for a screen or component).
- `js/feature-name.js` - Brief description (e.g., Behaviour for a screen).
- `js/mockData.js` - Brief description (e.g., Mock data used by the prototype).

### Notes

- Unit tests are not required for prototypes — skip test files unless the PRD specifically calls for them.
- If the project adds Node tooling (e.g. Supabase), check `package.json` for available scripts.
...
When the task list is complete, suggest:

> "The task list is ready! To start implementing, drag in `3. process-task-list.md` and reference this task list."
